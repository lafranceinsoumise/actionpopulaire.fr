...
class WebhookTestCase(FakeDataMixin, TestCase):
    ...
    def test_test_subscription(self, send_donation_email):
        ...
        self.assertEqual(Payment.objects.last().status, Payment.STATUS_COMPLETED)
        self.assertEqual(Payment.objects.last().subscription, subscription)
        self.assertEqual(Payment.objects.last().price, 1000)
        ...
        self.assertEqual(Payment.objects.last().status, Payment.STATUS_REFUSED)
        self.assertEqual(Payment.objects.last().subscription, subscription)
        self.assertEqual(Payment.objects.last().price, 1000)
