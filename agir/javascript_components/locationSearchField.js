...
module.exports = function (input) {
  ...
  document.addEventListener('click', function() { dropdownDiv.classList.remove('open'); });
  dropdownDiv.addEventListener('click', function(e) { e.stopPropagation(); });
  ...
  submitButton.addEventListener('click', function (event) {
    ...
    getResults(query).then(function (results) {
      if (results.length === 0) {
        ...
        results.slice(0, 10).forEach(function (d) {
          resultList.appendChild(createLinkElement(d.display_name, function (e) {
            e.preventDefault();
            setCoordinates(d);
      ...
      resultList.appendChild(createLinkElement('Erreur de connexion au serveur de géolocalisation... Réessayez plus tard.'));
      dropdownDiv.appendChild(resultList);
      dropdownDiv.classList.add('open');
      submitButton.disabled = false;
    });
