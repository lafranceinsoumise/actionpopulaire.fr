...
class CheckPaymentSearchForm(forms.Form):
    numbers = forms.CharField(
        label="Numéro(s) de chèque", required=True,
        help_text=_("Saisissez les numéros de transaction du chèque, séparés par des espaces"))
    amount = forms.DecimalField(label="Montant du chèque", decimal_places=2, min_value=0, required=True)
    ...
    def clean_numbers(self):
        ...
        try:
            ...
            raise forms.ValidationError(_("Entrez les numéros de chèque séparés par des espace"))
        ...
        if len(missing_checks) == 1:
            raise forms.ValidationError(ugettext("Le chèque n°{n} n'existe pas.").format(n=missing_checks[0]))
        elif missing_checks:
            raise forms.ValidationError(ugettext("Les paiements de numéros {numeros} n'existent pas.").format(
        ...
        with transaction.atomic():
            ...
            for payment in queryset.filter(status=CheckPayment.STATUS_WAITING):
                ...
                payment.events.append(
                    {'change_status': status, 'date': now, 'origin': 'check_payment_admin_action'}
                )
                payment.save()
...
class CheckPaymentAdmin(admin.ModelAdmin):
    ...
    def actions_buttons(self, object):
        if object.status == CheckPayment.STATUS_WAITING:
            ...
            return format_html_join(
                ...
                ((reverse('admin:checks_checkpayment_change_status', kwargs={'pk': object.pk, 'status': status}), label)
    ...
    def get_urls(self):
        return [
                   path('search/', admin_site.admin_view(self.search_check_view), name='checks_checkpayment_search'),
                   path('validate/', admin_site.admin_view(self.validate_check_view),
                        name='checks_checkpayment_validate'),
                   path('<int:pk>/change/<int:status>/', admin_site.admin_view(self.change_status),
    ...
    def validate_check_view(self, request):
        ...
        return_response = HttpResponseRedirect(reverse('admin:checks_checkpayment_search'))

        if not get_form.is_valid():
            return return_response

        amount, numbers = get_form.cleaned_data['amount'], get_form.cleaned_data['numbers']
        ...
        can_validate = (total_price == check_amount) and all(c.status == CheckPayment.STATUS_WAITING for c in payments)
        ...
        if can_validate and request.method == 'POST':
            ...
            with transaction.atomic():
                for p in payments:
                    ...
                    p.events.append(
                        {'change_status': CheckPayment.STATUS_COMPLETED, 'date': now,
        ...
        return TemplateResponse(
            ...
            context={
                'checks': payments, 'can_validate': can_validate, 'total_price': total_price,
                'check_amount': check_amount, 'opts': CheckPayment._meta,
            }
        )

    def change_status(self, request, pk, status):

        if status not in [CheckPayment.STATUS_COMPLETED, CheckPayment.STATUS_REFUSED, CheckPayment.STATUS_CANCELED]:
            raise Http404()
        ...
        with transaction.atomic():
            payment.status = status
            payment.events.append(
                {'change_status': status, 'date': now, 'origin': 'check_payment_admin_change_button'}
            )
            payment.save()
            notify_status_change(payment)

        return HttpResponseRedirect(reverse('admin:checks_checkpayment_change', args=[payment.pk]))
