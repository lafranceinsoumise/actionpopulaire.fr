...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('--tags-contains', dest='tags_contains', type=str, default=None)
        parser.add_argument('-c', '--count-goal', dest='goal', type=int, default=None)

    def handle(self, event_id, tags, tags_contains, goal, **kwargs):
        binaries = Person.objects.filter(draw_participation=True)\
                                         .exclude(gender=Person.GENDER_OTHER).count() / Person.objects.filter(
            draw_participation=True).count()
        ...
        for g, label in Person.GENDER_CHOICES:
            ...
            if goal is not None:
                gender_goal = round(goal*binaries/2) if g != Person.GENDER_OTHER else round(goal*(1-binaries))
                print(f"Il faut tirer {round((gender_goal-rsvp)*(tas/rsvp))} pour atteindre {gender_goal}.")
