...
from agir.people.token_buckets import SubscribeIPBucket, SubscribeEmailBucket, is_rate_limited_for_subscription
...
class BaseSubscriptionView(SimpleOpengraphMixin, FormView):
    ...
    def form_valid(self, form):
        if is_rate_limited_for_subscription(ip=self.request.META['REMOTE_ADDR'], email=form.cleaned_data['email']):
            form.add_error(field=None, error=ValidationError(subscription_rate_limite_message))
...
class ConfirmSubscriptionView(View):
    ...
    allowed_fields = [
        'email', 'location_zip', 'location_address1', 'location_address2', 'location_city', 'location_contry'
    ]
    ...
    def perform_create(self, params):
        try:
            ...
            messages.add_message(
                self.request,
                messages.INFO,
                self.error_messages['already_created']
            )
