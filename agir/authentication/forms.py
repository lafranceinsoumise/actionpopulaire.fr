...
class EmailForm(forms.Form):
    ...
    def send_email(self):
        if not send_mail_bucket.has_tokens(self.person.pk):
            self.add_error('email', forms.ValidationError(self.messages['rate_limited'], 'rate_limited'))
            return False

        self.short_code, self.expiration = short_code_generator.generate_short_code(self.person.pk)
        send_login_email.delay(self.cleaned_data['email'], self.short_code, self.expiration.timestamp())
        return True
...
class CodeForm(forms.Form):
    ...
    def clean_code(self):
        ...
        if not short_code_generator.is_allowed_pattern(code):
            raise forms.ValidationError(self.messages['incorrect_format'], 'incorrect_format')
