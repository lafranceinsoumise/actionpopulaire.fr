import { addQueryStringParams } from "./uri.js";

describe("URI utils", function () {
  describe("addQueryStringParams", function () {
    it("should return an empty string if argument uri is falsy", function () {
      const args = [null, {}];
      const expected = "";
      const result = addQueryStringParams(...args);
      expect(result).toEqual(expected);
    });
    it("should return argument uri if argument params is falsy", function () {
      const args = ["https://actionpopulaire.fr/", null];
      const expected = args[0];
      const result = addQueryStringParams(...args);
      expect(result).toEqual(expected);
    });
    it("should return argument uri if argument params type is not 'object'", function () {
      const args = ["https://actionpopulaire.fr/", "not an object"];
      const expected = args[0];
      const result = addQueryStringParams(...args);
      expect(result).toEqual(expected);
    });
    it("should return argument uri if argument params type has no properties", function () {
      const args = ["https://actionpopulaire.fr/", {}];
      const expected = args[0];
      const result = addQueryStringParams(...args);
      expect(result).toEqual(expected);
    });
    it("should add query string params to argument uri if none are already present", function () {
      const args = ["https://actionpopulaire.fr/", { a: "a" }];
      const expected = "https://actionpopulaire.fr/?a=a";
      const result = addQueryStringParams(...args);
      expect(result).toEqual(expected);
    });
    it("should add query string params to argument uri if some are present", function () {
