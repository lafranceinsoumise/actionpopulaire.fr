...
const groupTypes = [
  {
    ...
    description: <p>
      Les groupes d’action géographiques sont constitués sur la base d’un territoire réduit (quartier,
      ...
      que d’un seul groupe d’action géographique.
    </p>
  },
  {
    code: 'P',
    ...
    description: <p>
      Les groupes d’action professionnels rassemblent des insoumis⋅es qui souhaitent agir au sein de leur
      entreprise ou de leur lieu d’étude.
    </p>
  },
  {
    code: 'F',
    ...
    description: <p>
      Les groupes d’action fonctionnels sont des groupes d’action transversaux autour de fonctions précises
      (mise en place de formation, organisation des apparitions publiques, rédaction de tracts,
      chorale insoumise, journaux locaux, auto-organisation, etc…).
    </p>
  },
  {
    code: 'B',
    ...
    let steps = [
      {
        ...
        component: <GroupTypeStep setFields={this.setFields} fields={this.state.fields} subtypes={this.state.subtypes}/>
      },
      {
        name: 'Informations de contact',
        component: <ContactStep setFields={this.setFields} fields={this.state.fields}/>
      },
      {name: 'Localisation', component: <LocationStep setFields={this.setFields} fields={this.state.fields}/>},
    ...
    return (
      <div className="row padtopmore">
        ...
        <div className="col-sm-6 padbottom">
          {
            groupTypes.map((type, i) => <div key={type.code} className="type-selector">
              <button
              ...
              <Transition
                in={fields.type === type.code && this.subtypesFor(type.code).length > 1}
                timeout={1000}
                mountOnEnter unmountOnExit
              >
                {
                  state => {
                    const show = this.groupRefs[i].current && ['entering', 'entered'].includes(state);
                    return <div
                      className="subtype-selector" ref={this.groupRefs[i]}
                      style={{
                        height: show ? this.groupRefs[i].current.scrollHeight + 'px' : '0',
                      }}
                    >
                      <div>
                        <em>Choisissez maintenant les thèmes qui vous intéressent.</em>
                        <NavSelect
                          choices={this.subtypesFor(type.code).map(s => ({value: s.label, label: s.description}))}
                          value={fields.subtypes} max={3}
                          onChange={subtypes => this.props.setFields({subtypes})}
                        />
                      </div>
                    </div>;
                  }
    ...
    return (
      <div className="row padtopmore">
        <div className="col-md-6">
          ...
          <ul>
            <li>
              <strong>Type de groupe&nbsp;:</strong> {groupTypes.find(t => t.code === fields.type).label}
            </li>
            <li>
              <strong>Numéro de
        ...
        <div className="col-md-6">
          ...
          <form onSubmit={this.post}>
            <div className="form-group">
              <input
                className="form-control" ref={i => this.groupName = i} type="text" placeholder="Nom du groupe"
                required
              />
            </div>
