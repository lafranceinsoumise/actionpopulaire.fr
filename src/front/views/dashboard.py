...
class DashboardView(SoftLoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        rsvped_events = Event.objects.upcoming().filter(attendees=person)
        members_groups = SupportGroup.objects.filter(memberships__person=person)

        suggested_events = [
            (event, 'Cet événément est organisé par un groupe dont vous êtes membre.')
            for event in Event.objects.upcoming().filter(Q(organizers_groups__in=person.supportgroups.all()) & ~Q(attendees=person))
        ]
        last_events = Event.objects.past().filter(attendees=person)[:10]

        organized_events = Event.objects.filter(organizers=person)
